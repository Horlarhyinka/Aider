...
export const addMarker = (map: any, maps: any, position: Coordinate, template: React.JSX.Element | string, bounce=false, isResponder= false)=>{
    const marker = new maps.Marker({ position, map, clickable: true })
    if(bounce){
        marker.setAnimation(maps.Animation.BOUNCE)
    }
    isResponder && marker.setIcon({fillColor: "green", path: marker_path, strokeColor: "white", fillOpacity: 1})
